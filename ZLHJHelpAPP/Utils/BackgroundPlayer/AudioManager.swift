...
class AudioManager: NSObject {
    ...
    private func setupBackgroundAudioPlayer() {
        do {
            self.backgroundAudioPlayer = try AVAudioPlayer(contentsOf: URL(fileURLWithPath: Bundle.main.path(forResource: "no_notice", ofType: ".mp3")!))
            debugPrint(Bundle.main.path(forResource: "no_notice", ofType: ".mp3")!)
        } catch let error {
            debugPrint("\(type(of:self)):\(error)")
        }
